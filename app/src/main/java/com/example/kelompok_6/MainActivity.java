...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtHome = findViewById(R.id.home_team);
        txtAway = findViewById(R.id.away_team);
    ...
    public void handlerNext(View view) {
        ...
        if(txtHome.getText().toString() == txtAway.getText().toString()){
            Toast.makeText(MainActivity.this, "Nama Team Tidak boleh sama", Toast.LENGTH_SHORT).show();
        }else  if (txtHome.getText().toString().length()==0){
            txtHome.setError("Tidah Boleh Kosong");
            return;
        }else  if (txtAway.getText().toString().length()==0) {
            txtAway.setError("Tidah Boleh Kosong");
            return;
        }else {
